...
class ActSpec extends AnyFlatSpec with Matchers:
  ...
  "A team" should "move when he has to" in:
    ...
    val p1   = Player(0, initial, movement = Movement.still, action = Move(defaultDirection, MatchConfig.playerSpeed))
    val p2   = Player(1, initial, movement = Movement.still, action = Move(defaultDirection, MatchConfig.playerSpeed))
  ...
  "A player" should "keep last movement if he has no new action" in:
    ...
    val player       = Player(0, Position(0, 0), movement = lastMovement, action = Initial)
    player.updateMovement().movement should be(lastMovement)

  it should "stand still if he is stopped" in:
    val player = Player(0, Position(0, 0), action = Stopped(1))
    player.updateMovement().movement should be(Movement.still)

  it should "move when he has to" in:
    val player = Player(0, Position(0, 0), action = Move(defaultDirection, MatchConfig.playerSpeed))
  ...
  it should "stand still when hitting the ball" in:
    ...
    val player    = Player(0, Position(0, 0), movement = notStill, ball = Some(ball), action = Hit(direction, speed))
    player.updateMovement().movement should be(Movement.still)
  ...
  it should "move with the player that controls it" in:
    val ball    = Ball(Position(0, 0))
    val carrier = Player(0, Position(0, 0), ball = Some(ball), action = Move(defaultDirection, MatchConfig.playerSpeed))
  ...
  it should "move with the player that took it" in:
    ...
    val carrier        = Player(0, Position(0, 0), playerMovement, action = Take(ball))
