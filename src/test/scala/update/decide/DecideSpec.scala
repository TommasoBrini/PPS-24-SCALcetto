...
class DecideSpec extends AnyFlatSpec with Matchers:
  ...
  it should "assign markings between defenders and attackers" in:
    ...
    val updatedState    = decide(state)
    val updatedAttacker = updatedState.teams.find(_.hasBall).get.players.head
    val updatedDefender = updatedState.teams.find(!_.hasBall).get.players.head

    updatedAttacker.decision should not be Decision.Initial
    updatedDefender.decision should not be Decision.Initial

  it should "throw IllegalArgumentException for invalid team count" in:
    val player       = Player(1, Position(5, 5), Movement.still)
    val team         = Team(1, List(player), hasBall = true)
    val ball         = Ball(Position(0, 0), Movement.still)
    val invalidState = MatchState(List(team), ball) // Solo una squadra

    an[IllegalArgumentException] should be thrownBy decide(invalidState)

  it should "handle empty teams gracefully" in:
    val emptyTeam1 = Team(1, List(), hasBall = true)
    val emptyTeam2 = Team(2, List(), hasBall = false)
    val ball       = Ball(Position(0, 0), Movement.still)
    val state      = MatchState(List(emptyTeam1, emptyTeam2), ball)

    val updatedState = decide(state)

    updatedState.teams should have size 2
