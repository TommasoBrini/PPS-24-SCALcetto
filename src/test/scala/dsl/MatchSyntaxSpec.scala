...
class MatchSyntaxSpec extends AnyFlatSpec with Matchers:
  ...
  "isBallOut" should "be true if the ball is out" in:
    val state = MatchState((Team(Nil), Team(Nil)), Ball(Position(UIConfig.fieldWidth + 1, UIConfig.fieldHeight + 1)))
    state.isBallOut should be(true)

  it should "be false when ball is in game" in:
    val state = MatchState((Team(Nil), Team(Nil)), Ball(Position(UIConfig.fieldWidth / 2, UIConfig.fieldHeight / 2)))
